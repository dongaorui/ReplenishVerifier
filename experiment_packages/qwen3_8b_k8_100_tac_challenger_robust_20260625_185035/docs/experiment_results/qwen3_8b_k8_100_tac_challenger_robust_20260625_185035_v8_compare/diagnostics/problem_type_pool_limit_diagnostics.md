...
| Best-of-K | multi_item_capacity | 20 | 0.6 | 0.55 | 0.04999999999999993 | True | candidate-pool limitation: oracle@k is low, so failures are bounded by candidate generation/repair quality rather than selector choice alone | True |
| Best-of-K | single_item_multi_period | 20 | 1.0 | 0.95 | 0.050000000000000044 | False | selector limitation: oracle@k is available above selector accuracy, so no-reference ranking should be inspected | True |
...
| Solver only | multi_item_capacity | 20 | 0.6 | 0.6 | 0.0 | True | candidate-pool limitation: oracle@k is low, so failures are bounded by candidate generation/repair quality rather than selector choice alone | True |
...
| Solver only | single_item_multi_period_shortage | 20 | 1.0 | 0.8 | 0.19999999999999996 | False | selector limitation: oracle@k is available above selector accuracy, so no-reference ranking should be inspected | True |
| Solver only | single_period_newsvendor | 20 | 0.7 | 0.7 | 0.0 | False | selector is near the post-hoc oracle@k ceiling for this problem type | True |
