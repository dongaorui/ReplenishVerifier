...
这些 graph-based detectors 只提供 weak evidence，不替代原有结构规则；它们用于增强 certificate 的 evidence 字段和提高真实 LLM 输出中的弱鲁棒性。

### 4.3 Problem-type-specific Required Structure Schema

系统新增 problem-type-specific schema，将结构分为 required 与 optional。主 `structure_score` 只对 required structures 计算；optional structures 只报告，不进入主分数。例如：
...
这一设计避免 optional structure 被误计入主分数，也让不同 problem type 的结构要求更清晰。
...
如果规则未通过，certificate 会给出 `missing_reason` 与 `repair_hint`。因此，结构验证结果既可用于 ranking，也可用于解释、case study、repair prompt 和 preference data construction。
...
3. **LP artifact context and graph evidence.** 例如 binary declarations、objective 中变量出现、constraint expression 中变量组合，以及 LPStructureGraph 的 weak evidence。
...
其中，\(E(c)\) 表示代码可执行，\(Z(c)\) 表示 solver 返回 `Optimal`，\(SC(c)\) 是 required structures 的结构完整性，\(Sem(c)\) 是根据缺失结构得到的语义一致性信号。该分数不使用 `reference_objective`。
...
当候选缺失结构时，ReplenishVerifier 生成结构化反馈。例如：
...
只有在真实 LLM repair candidates 已生成并重新评估后，才能将 ReplenishVerifier-Repair 写成真实 repair 结果。
...
2. **Structure correctness 不等于完整 mathematical correctness。** 候选模型可能包含所有 high-level structures，但仍然存在错误系数、错误索引、边界条件错误或初始库存处理错误。
