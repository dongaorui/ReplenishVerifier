...
## 2026-06-24 — TypeAware-Consensus safe consensus hardening

`ReplenishVerifier-TypeAware-Consensus` now keeps its public method name but internally treats objective consensus as unsafe until candidate-observable verifier signals support it.

Key selection signals:

- `consensus_cluster_support`: raw candidate objective-cluster support.
- `candidate_quality_score`: no-reference quality discount from constraint coverage, objective-term coverage, LP coefficient sanity, LP health, structure score, type-aware/static validation, and text-triggered gates.
- `safe_consensus_score`: raw consensus multiplied by candidate-quality support.
- `wrong_consensus_risk`: raw consensus minus safe consensus; high values flag clusters that are large but structurally/objectively unsafe.
- `text_triggered_hard_gate_failures`: problem-type-scoped failures for capacity, shortage, fixed-order cost, binary order, and Big-M cues.

Important behavior:

- Raw consensus can still win when candidate quality is safe.
- A large objective cluster can be demoted when it misses capacity constraints, shortage/fixed-order/Big-M structures, LP/objective terms, or text-triggered hard gates.
- Capacity text gates are scoped to `multi_item_capacity`; non-capacity problem types are not penalized for lacking capacity constraints.
- Objective-term coverage now participates directly in safe consensus, so a shared objective that omits fixed order, shortage, holding, or ordering terms is treated as risky rather than blindly trusted.

New diagnostics / paper metrics:

- `wrong_consensus_risk.csv/.md` reports selected rows where raw consensus is materially discounted.
- `hard_subset_stress_test.csv/.md` summarizes capacity/shortage/fixed-order hard subsets by method.
- `full_vs_typeaware_consensus_diff.csv/.md` remains a post-hoc explanation of selection differences.
- `table_by_problem_type.*` now includes safe-consensus and wrong-consensus risk means.
- `table_hard_subset_stress.*` is generated by `build_paper_metrics`.

No-reference invariant:

Formal selection components do not include `reference_objective`, `objective_correct`, `relative_error`, oracle fields, reference LP, or reference answers. Post-hoc objective correctness appears only in diagnostics/evaluation tables.

Verification:

- Focused selector/diagnostic/leakage suite: `73 passed`.
